...
export class PostService {
  ...
  protected convertDateFromClient(post: IPost): IPost {
    const copy: IPost = Object.assign({}, post, {
      datePub: post.datePub != null && post.datePub.isValid() ? post.datePub.format(DATE_FORMAT) : null,
      time: post.time != null && post.time.isValid() ? post.time.toJSON() : null
    });
    return copy;
  }
  ...
  protected convertDateArrayFromServer(res: EntityArrayResponseType): EntityArrayResponseType {
    if (res.body) {
      res.body.forEach((post: IPost) => {
        post.datePub = post.datePub != null ? moment(post.datePub) : null;
        post.time = post.time != null ? moment(post.time) : null;
      });
    }
    return res;
  }
